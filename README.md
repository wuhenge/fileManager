...
> ⚠️ **非官方项目** - 本工具仅供学习交流使用，与任何商业或官方产品无关联。

文件管家是一个基于 React + Node.js 的文件监控与清理工具，专为飞牛OS系统设计，提供监控目录管理、文件名清理、文件移除、下载目录识别和操作日志查看等能力。
...
项目设计用于飞牛OS应用生态，构建后前端资源输出到 `app/ui`，由后端静态托管，并配合 `manifest`、`cmd`、`wizard` 等目录用于打包和部署。
...
- 使用本工具进行文件监控、清理、移除等操作所产生的任何后果均由使用者本人承担
- 使用者应确保其行为符合所在地区法律法规及服务提供商的使用条款
- 本工具不提供任何明示或暗示的保证，使用者需自行评估风险并采取必要的预防措施

如您使用本工具，即表示已阅读并同意上述声明。

## 隐私说明

本工具为本地运行的应用，**不会收集、传输或上传任何用户数据**。

- 所有配置、日志和操作记录仅存储在本地设备
- 不包含任何联网上报或统计功能
- 不使用任何第三方分析服务
- 使用者的文件结构和操作行为不会被分享或泄露
